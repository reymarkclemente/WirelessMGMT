...
| NetAdapter 2    | __Bridged (Replicate)__ | 10.51.1.0 /24 |
...
conf t
 ...
 vlan 10
  ...
  exit
 int vlan 1
  ip add 10.51.1.7 255.255.255.0
  no shut
 int g1
  no switchport
 ...
 ip route 10.0.0.0 255.0.0.0 10.51.1.4
 ip route 200.0.0.0 255.255.255.0 10.51.1.4
 !
 !
...
The __Wireless Management IP__ is __10.51.1.7__
...
## Exercise 01: Troubleshoot `Wireless-51` wifi so that it becomes operable in the network.
...
### 10. Assign the DHCP server for the WLAN, `10.51.10.4`, finally `Apply to Device`.
...
### 13. Set the Name and Description to `AP-WLANs`. Then, under WLAN-POLICY Maps, `Add` a mapping and set the WLAN Profile to `Wireless-51` with a Policy Profile `Wireless-POL`. Then, `Apply to Device`.
...
conf t
 ...
 int vlan 11
  ...
  ip add 10.51.11.4 255.255.255.0
  ip ospf 1 area 0
  no shut
 !
 ip dhcp excluded-address 10.51.11.1 10.51.11.100
 ip dhcp pool SOCPOOL
  network 10.51.11.0 255.255.255.0
  default-router 10.51.11.4 255.255.255.0
...
### Exercise 03: Create WLANs for `THREAT-HUNTERS` on VLAN 12. Using the network address 10.51.12.0/24
...
conf t
 ...
 int vlan 12
  ...
  ip add 10.51.12.4 255.255.255.0
  ip ospf 1 area 0
  no shut
 !
 ip dhcp excluded-address 10.51.12.1 10.51.12.100
 ip dhcp pool THUNTPOOL
  network 10.51.12.0 255.255.255.0
  default-router 10.51.12.4 255.255.255.0
